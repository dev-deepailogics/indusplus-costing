...
import type { BOMFabricItem } from "../types";

interface StyleFabricTabProps {
  formFabric: BOMFabricItem[];
  onFormFabricChange: (fabric: BOMFabricItem[]) => void;
  exchangeRate: number;
}

export function StyleFabricTab({
  formFabric,
  onFormFabricChange,
  exchangeRate,
}: StyleFabricTabProps) {
  function updateItem(idx: number, patch: Partial<BOMFabricItem>) {
    const next = [...formFabric];
    next[idx] = { ...next[idx], ...patch };
    onFormFabricChange(next);
  }
  ...
  function removeRow(idx: number) {
    onFormFabricChange(formFabric.filter((_, i) => i !== idx));
  }

  return (
    <div className="space-y-3">
      <p className="text-xs text-muted-foreground mb-2">
        Configure Fabric requirements (rates in USD. Conversion rate is {exchangeRate} PKR/$).
      </p>
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      updateItem(idx, {
                        ...
                        fabricCostPKR: cons * item.ratePKR,
                      });
                    }}
                  />
                </TableCell>
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      updateItem(idx, {
                        rateUSD,
                        ratePKR,
                        fabricCostPKR: item.consumptionPerPc * ratePKR,
                      });
                    }}
                  />
                </TableCell>
