...
function parseBound(
  val: string,
  isFrom: boolean,
): { val: number; inclusive: boolean; isValid: boolean } {
  const trimmed = (val || "").trim();
  if (!trimmed || trimmed === "-" || trimmed === "null") {
    return {
      val: isFrom ? -Infinity : Infinity,
      inclusive: true,
      isValid: true,
    };
  }
  ...
  if (trimmed.startsWith(">=")) {
    ...
    return {
      val: isNaN(num) ? (isFrom ? -Infinity : Infinity) : num,
      inclusive: true,
      isValid: !isNaN(num),
    };
  }
  ...
  return {
    val: isNaN(parsed) ? (isFrom ? -Infinity : Infinity) : parsed,
    inclusive: true,
...
function validateSamRanges(
  ...
  if (!isSamRange)
    return { isValid: true, errors: [], conflictingRowIds: new Set() };
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (!fromBound.isValid) {
      errors.push(
        `"${label}": Invalid 'SAM/PC From' value ("${rawFrom}"). Only numbers or operators (e.g. 19, >=37, -) are allowed.`,
      );
      conflictingRowIds.add(r.id);
    }
    if (!toBound.isValid) {
      errors.push(
        `"${label}": Invalid 'SAM/PC To' value ("${rawTo}"). Only numbers or operators (e.g. 23, <=18, -) are allowed.`,
      );
      conflictingRowIds.add(r.id);
    }
    ...
    if (fromBound.isValid && toBound.isValid && fromBound.val > toBound.val) {
      errors.push(
        `"${label}": 'From' (${rawFrom}) cannot be greater than 'To' (${rawTo}).`,
      );
      conflictingRowIds.add(r.id);
    }
...
export function SimpleTableEditor({
  ...
  const [activeConfirmCardId, setActiveConfirmCardId] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    if (!cards.some((c) => c.id === selectedCardId)) {
      const fallback =
        cards.find((c) => c.isActive)?.id || cards[0]?.id || "card-1";
  ...
  async function handleManualSave() {
    if (!samValidation.isValid) {
      toast.error(
        samValidation.errors[0] ||
          "Overlapping or invalid SAM ranges detected. Please fix before saving.",
      );
      return;
    }
    ...
    try {
      ...
      const sanitizedCards = cards.map((c) => ({
        ...
        rows: c.rows.map((r, idx) => ({
          ...
          values: {
            ...r.values,
            ...(firstKey &&
            (!r.values[firstKey] || r.values[firstKey].trim() === "") &&
            templateC?.rows[idx]?.values[firstKey]
      ...
      const sanitizedActiveC =
        sanitizedCards.find((c) => c.id === activeC.id) || sanitizedCards[0];
  ...
  async function handleCreateCard(cardName: string) {
    ...
    const templateCard =
      cards.find((c) => c.isActive) || cards[0] || currentCard;
    const templateRows =
      templateCard.rows && templateCard.rows.length > 0
        ? templateCard.rows
        : currentRows;
  ...
  function handleAddRow(rowName: string) {
    ...
    if (
      currentRows.some(
        (r) =>
          (r.values[firstKey] || "").trim().toLowerCase() ===
          trimmed.toLowerCase(),
      )
    ) {
  ...
  function handleDeleteRow(rowId: string) {
    ...
    setDeleteRowId(null);
    toast.info(
      "Row removed from draft. Click Save Changes to apply to Database.",
    );
  }
  ...
  return (
    <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="flex items-start justify-between gap-2 mb-2">
                    ...
                    {card.isActive ? (
                      ...
                      <Badge
                        variant="outline"
                        className="text-[10px] text-muted-foreground px-1.5 py-0.5 shrink-0"
                      >
                        Inactive
                      </Badge>
                    )}
                  ...
                  <div className="text-xs text-muted-foreground mb-3 flex items-center gap-1.5">
                    ...
                    <span>
                      {rowCount} {rowCount === 1 ? "row" : "rows"} configured
                    </span>
                  </div>
                </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Viewing and editing parameters for {currentCard.name}.{" "}
                {hasChanges && (
        ...
        {!samValidation.isValid && (
          <div className="rounded-lg border border-rose-300 dark:border-rose-900 bg-rose-50/95 dark:bg-rose-950/40 p-3 text-xs text-rose-900 dark:text-rose-200 space-y-1.5 shadow-2xs">
            <div className="flex items-center gap-2 font-bold text-rose-700 dark:text-rose-400">
              ...
              <span>
                Invalid Range Configuration: Overlapping Boundaries Detected
              </span>
            </div>
          ...
          <Table>
            ...
            <TableBody>
              {currentRows.map((row, rowIdx) => {
                const currentUseType = (
                  row.values.useType ||
                  row.values.use_type ||
                  "sam"
                ).toLowerCase();
                const isPiece =
                  currentUseType === "piece" || currentUseType === "perpiece";
                const isSam = !isPiece;
                const templateCard = cards[0];
                const isConflicting = samValidation.conflictingRowIds.has(
                  row.id,
                );
                  ...
                  <TableRow key={row.id}>
                    {currentColumns.map((col, colIndex) => {
                      ...
                      const cellValue =
                        isDescription &&
                        (!row.values[col.key] ||
                          row.values[col.key].trim() === "")
                          ? templateCard?.rows[rowIdx]?.values[col.key] ||
                            row.values[col.key] ||
                            ""
                          : (row.values[col.key] ?? "");
      ...
      <AlertDialog
        open={activeConfirmCardId !== null}
        onOpenChange={(open) => !open && setActiveConfirmCardId(null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              {cards.find((c) => c.id === activeConfirmCardId)?.name}" as the
              active card? This card will be used in the Cost Sheet
              calculations.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
